...
static bool g_init=false;static uint8_t g_own=BLE_OWN_ADDR_PUBLIC;static SemaphoreHandle_t g_sync=nullptr;static SemaphoreHandle_t g_bleMutex=nullptr;static NimBLEScan g_scan;
...
NimBLEScanResults NimBLEScan::getResults(uint32_t ms,bool){NimBLEDevice::init("AndersonHome-Bridge");if(g_bleMutex)xSemaphoreTake(g_bleMutex,portMAX_DELAY);results_.items_.clear();if(g_sync)xSemaphoreTake(g_sync,0);ble_gap_disc_params p{};p.passive=active_?0:1;p.itvl=interval_;p.window=window_;p.filter_duplicates=1;int rc=ble_gap_disc(NimBLEDevice::ownAddrType(),(int32_t)ms,&p,&NimBLEScan::gapEvent,this);if(rc==0&&g_sync)xSemaphoreTake(g_sync,pdMS_TO_TICKS(ms+1000));ble_gap_disc_cancel();if(g_bleMutex)xSemaphoreGive(g_bleMutex);return results_;}
