...
static constexpr const char* ANDERSON_FIRMWARE_VERSION="3.1.51";
...
String stateJson(){
  ...
  tm l{};if(timeValid()){time_t n=time(nullptr);localtime_r(&n,&l);uint16_t dawn=scheduler.civilDawnMinutes(l),dusk=scheduler.civilDuskMinutes(l);cfg["dawn"]=fmtTime(dawn);cfg["dusk"]=fmtTime(dusk);String s1Start=s.schedule1StartAtDusk?(String("dusk (")+fmtTime(dusk)+")"):fmtTime(s.onMinutes),s2End=s.schedule2EndAtDawn?(String("dawn (")+fmtTime(dawn)+")"):fmtTime(s.schedule2EndMinutes);d["scheduleWindow"]=String("Schedule 1 ")+s1Start+" - "+fmtTime(s.offMinutes)+" • Schedule 2 "+fmtTime(s.offMinutes)+" - "+s2End+" at "+String(s.schedule2Brightness)+"%";d["nextEvent"]=scheduler.nextEventLabel(l);
    ...
    if(!scheduledCustom){scheduledTheme=scheduler.resolve(tonight);for(size_t i=0;i<EVENT_COUNT&&i<MAX_BUILTIN_EVENTS;i++)if(eventAllowedInActiveSchedule(i)&&scheduledTheme.name==EVENTS[i].name){scheduledIndex=(int)i;break;}
      if(scheduledIndex<0){for(size_t i=0;i<EVENT_COUNT&&i<MAX_BUILTIN_EVENTS;i++){if(!eventAllowedInActiveSchedule(i)||EVENTS[i].rule==RuleType::Month)continue;if(eventActiveOn(i,tonight)){scheduledIndex=(int)i;break;}}}
      if(scheduledIndex<0&&(s.leadDays||s.trailDays)){for(size_t i=0;i<EVENT_COUNT&&i<MAX_BUILTIN_EVENTS;i++){if(!eventAllowedInActiveSchedule(i)||EVENTS[i].kind!=EventKind::Holiday)continue;if(eventWindowActiveOn(i,tonight,s.leadDays,s.trailDays)){scheduledIndex=(int)i;break;}}}
      if(scheduledIndex<0){for(size_t i=0;i<EVENT_COUNT&&i<MAX_BUILTIN_EVENTS;i++){if(!eventAllowedInActiveSchedule(i)||EVENTS[i].rule!=RuleType::Month)continue;if(eventActiveOn(i,tonight)){scheduledIndex=(int)i;break;}}}}
    JsonObject scheduled=d["scheduledEvent"].to<JsonObject>();if(scheduledCustom){scheduled["name"]=scheduledTheme.name;scheduled["id"]="";scheduled["enabled"]=true;scheduled["toggleable"]=false;scheduled["custom"]=true;}else if(scheduledIndex>=0){scheduled["name"]=EVENTS[scheduledIndex].name;scheduled["id"]=EVENTS[scheduledIndex].id;scheduled["enabled"]=eventStateEnabled((size_t)scheduledIndex);scheduled["toggleable"]=true;scheduled["custom"]=false;}else{scheduled["name"]="No scheduled event";scheduled["id"]="";scheduled["enabled"]=false;scheduled["toggleable"]=false;scheduled["custom"]=false;}
