...
static constexpr const char* ANDERSON_FIRMWARE_VERSION="3.1.30";
...
static bool pinMatchesStoredProfile(const String& profile,const String& pin){if(!fourDigitPin(pin)||profileRole(profile)==ROLE_NONE)return false;const String& salt=profile=="jason"?jasonPinSalt:(profile=="kelly"?kellyPinSalt:shirleyPinSalt);const String& expected=profile=="jason"?jasonPinHash:(profile=="kelly"?kellyPinHash:shirleyPinHash);return pinRecordConfigured(salt,expected)&&constantTimeEqual(pinDigest(profile,pin,salt),expected);}
...
bool eventAllowedInActiveSchedule(size_t i){return i<EVENT_COUNT&&eventColorThemeIncludesEvent(activeEventColorTheme,i);}
