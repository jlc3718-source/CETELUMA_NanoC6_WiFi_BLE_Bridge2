...
s=s.replace(panel,panel+'<div class="panel"><strong>Saved Custom Lights</strong><div class="sub">Named setups saved on the NanoC6. Apply them anytime or add them to the automatic schedule.</div><div id="customPresetList" class="presetList"></div></div>',1)
...
s=s.replace("if(btn.dataset.tab==='events')loadEvents();","if(btn.dataset.tab==='events'){loadEvents();loadCustomSchedules();}",1)
