...
s=s.replace('Routine updates can be installed here over local Wi-Fi. Select the app-only BIN; the NanoC6 writes it to the inactive OTA slot and keeps the current firmware available as the previous slot.',
            'Select the app-only BIN, then press Upload, Install & Reboot. The status display shows each update stage while the NanoC6 writes the update to the inactive OTA slot, verifies it, reboots, reconnects and refreshes this page.',1)
s=s.replace('Upload & Stage Update','Upload, Install & Reboot',1)
s=s.replace('Local Wi-Fi only. The device will not reboot until you press Reboot NanoC6.','Updates install automatically. The status above shows each stage and the NanoC6 reboots when verification completes.',1)
s=s.replace('Remote firmware update enabled for Jason. The device will not reboot until you press Reboot NanoC6.','Updates install automatically. The status above shows each stage and the NanoC6 reboots when verification completes.',1)
s=s.replace('Firmware verified and staged. Press Reboot NanoC6 when ready.','Firmware verified. New boot slot selected. Rebooting NanoC6…')
